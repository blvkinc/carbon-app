import { ArrowLeft, X, Wallet, Gift } from "lucide-react";
...
export default function TrainingPayment() {
    ...
    const bookingData = location.state || {}; // Includes totalPrice and rewardsPoints
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white p-4 pb-safe z-20 space-y-3">
                <div className="mx-auto max-w-md space-y-3">
                    <div className="w-full bg-[#ECFDF5] rounded-xl p-3 flex items-center justify-center gap-2">
                        ...
                        <span className="text-xs font-bold text-[#10B981]">You'll Earn {bookingData?.rewardsPoints || 100} Carbon loyalty points</span>
                    </div>

                    <Button
                        className="w-full h-12 rounded-lg bg-black text-white hover:bg-gray-900 font-bold text-sm"
                        onClick={() => navigate("/training/success", { state: bookingData })}
                    >
                        Continue
                    </Button>
                </div>
